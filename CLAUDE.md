...
- **TilemapRenderer.js** - Sparse chunk-based world renderer. `generateChunkMap()` creates the initial 3×4 grid using `chunkTiles` Map (sparse storage). Supports `setTileAt()`/`getTileAt()` for runtime tile modification. Renders multi-layer TMX maps (house.tmx ground/roof layers, shed1.tmx ground/roof layers). `renderGroundLayers()` renders both house and shed ground layers; `renderRoofLayers()` renders both house and shed roof layers (shed roof never hidden). `renderGreatPath()` draws the y=30–33 path strip separately. `isCustomTilemapTile()` covers new house footprint and shed footprint (x=25–29, y=38–41) to block flower spawning. `shedGroundLayers`, `shedRoofLayers` arrays initialized in constructor; loaded in `generateChunkMap()` from shed1.tmx alongside house.tmx (fetched in parallel via `Promise.all`). **Performance**: tile render loop caches the last chunk key to avoid redundant `chunkTiles.has()` string-concat lookups across same-chunk columns. `invalidateBridgeCache()` clears the north bridge column Set cache — called by `_onPathChanged()` whenever path tiles change.
...
- **EffectUtils.js** - Shared floating harvest/resource effect utilities. Functions: `withAlpha(ctx, alpha, drawFn)` — applies globalAlpha only when alpha < 1 (skips save/restore entirely at full opacity). `createHarvestEffect(x, y, tileId)` → effect object (duration from `CONFIG.effects.floatingDuration`). `updateEffects(effects, deltaTime)` → mutates array in-place using mark-and-sweep (no splice-in-loop). `renderEffects(ctx, effects, tilesetImage, getTilesetSourceRect, tileSize)` → single save/restore; sets font/stroke/lineWidth/textAlign once, only changes globalAlpha and fillStyle per effect. `renderEffectsMulti(ctx, effectArrays, tilesetImage, getTilesetSourceRect, tileSize)` → one save/restore total across multiple effect arrays — used by Game.render() to batch all floating text in one pass. Imported by ResourceManager base class and ForestGenerator.
- **ResourceManager.js** - Shared base class for entity managers that track a list of resources (trees, ore veins, crops). Provides: unified `this.effects` array (updated/rendered same way across subclasses), mark-and-sweep `_cleanupGone()` hook, template-method pattern. `update(deltaTime)` calls `_updateResources()` → `_cleanupGone()` → `updateEffects()`. `renderEffects(ctx, camera)` delegates to EffectUtils. `getResourceAt(tileX, tileY)` — base lookup (iterate resources, skip isGone, call containsTile); subclasses expose domain aliases as one-liner wrappers. `setDependencies(deps)` — `Object.assign(this, deps)` for bulk dependency injection. `_withAlpha(ctx, alpha, drawFn)` — delegates to EffectUtils.withAlpha. Subclasses assign `this.resources` alias (e.g. `this.crops = this.resources`) and override `_updateResources()` if needed. Extended by CropManager, TreeManager, OreManager.

### Chunk World System (js/)

- **ChunkManager.js** - Sparse dynamic chunk grid. Stores chunks in a `Map("col,row" → chunk)`. Initial 3×4 grid = 12 chunks. Chunk states: `OWNED`, `TOWN`, `PURCHASABLE`, `LOCKED`. Chunk types: `FARM`, `TOWN`, `FOREST`. Key methods: `initialize()`, `purchaseChunk()` (forest chunks deduct gold via `inventory.spendGold()`; town plots deduct villager milestone count via `_townChunkThreshold()`; returns false if can't afford), `getChunkPrice(col, row)` (Manhattan distance from farm chunk → CONFIG.chunks.purchasePrices), `getChunkForTile()`, `isPlayerOwned()`, `isTownChunk()`, `isAccessible()`, `getChunkBounds()`, `render()` (borders), `renderPurchaseSigns()` (shows price in gold or red if unaffordable). Fires `onChunkPurchased` callback. Holds pluggable `generatorRegistry`. Holds `inventory` reference and `game` reference (set by Game.init()). `serializeChunks()` — returns plain array of `{col,row,type,state,isTownPlot,purchasedAsTown}` snapshots (used by SaveManager instead of reaching into `chunks` Map directly). **Town expansion**: TOWN chunks promote LOCKED neighbors north of farmRow as `isTownPlot:true` PURCHASABLE chunks — claimed with housed-villager milestones rather than gold. Threshold sequence: `_townChunkThreshold(n)` = 1, 3, 6, 9, 12 … (`townExpansionBase: 1`, `townExpansionIncrement: 3`). `game.townChunksUnlocked` tracks how many town plots have been claimed. North forest flanking chunks (rows 0–1): permanently LOCKED, never purchasable.
...
- **UIManager.js** - Menu system. `activeMenu` tracks current open menu (null/'storage'/'crafting'/'shop'/'well'/'building'/etc.). Key methods: `openWellMenu(game)` / `refreshWellMenuIfOpen(game)` — well menu now uses standard overlay (not static DOM). `openBuildingMenu(building, def, onDeconstruct)` — opens building management panel for placed buildings (shows state, Deconstruct button). Panels:
...
- **Well.js** - Well structure at world tile (23, 39): 2-tile wide × 3-tile tall. Top row (tiles 1256–1257, y=39) visual-only rendered above characters; middle + bottom rows (tiles 1320–1321 / 1384–1385, y=40–41) block movement. `isObstacle(x,y)` checks middle+bottom rows. `getAdjacentServiceTile()` → (23, 41). `registerInteractable()` registers click area for `'openWell'` action. Rendered in depth-sorted pass (`getSortY()`) + upper-layers pass (`renderTop()`). Well menu (`#well-menu`) opened by click, has Fill buttons for human/goblin cans. `game.wateringCanWater/Max` and `game.goblinWaterCanWater/Max` (all default 20).
- **CropManager.js** / **Crop.js** - Extends `ResourceManager` (`this.crops = this.resources`). Crop lifecycle: 5 growth stages (variable time per crop), harvest with floating "+1" feedback via `this.effects`, post-harvest decay effects. Overrides `_updateResources(deltaTime)` to apply shrine growth-speed multiplier before passing to each crop. Multi-step watering state machine: `wateringState` ∈ `'needs_water' | 'watering_cooldown' | 'growing'`; `wateringsPerStage` per crop type (1 for most, 2 for PUMPKIN/WHEAT/SUNFLOWER); 30s cooldown between multi-waterings. Crop visual feedback uses tile ID: dry ground tile (818) when `needs_water`, wet ground tile (882) when growing/cooldown — no overlay dots. Wild crops (startAsPlanted=false) begin in `'growing'` state, never need watering. `get isWatered()` backward-compat getter. `renderCropGroundTile(ctx, crop)` — renders only the ground-layer tiles (`tile.isGround`) for a single crop; called inline in depth-sort pass by Game.render() (replaces the old `renderAllCropGroundTiles()` pre-pass). Phase 3: `setGame(game)` called after init; `_getGrowthSpeedMultiplier()` reads `game.homeUpgrades.shrineUpgrades.fertileSoilLevel` (0=1.0×, 1=0.85×, 2=0.70×); `update()` uses `effectiveDt = dt * speedMultiplier` for stage advancement.
...
- **FlowerManager.js** - Spawning and management of flowers and weeds. Dynamic spawn rate based on grass coverage. 75% weeds vs 25% flowers. `_getSpawnAreas()` returns farm grass, both town chunks (store + home), and all allocated forest chunk areas; result is cached in `_spawnAreasCache` — call `invalidateSpawnAreasCache()` after chunk purchase. `setDependencies(deps)` — `Object.assign(this, deps)` for bulk dependency injection. `_flowerTileMap` — `Map<"x,y", Flower>` maintained on spawn/cleanup for O(1) `getFlowerAt()` lookup (replaces O(n) array scan). `harvestEffects` and `weedClickEffects` use mark-and-sweep cleanup. `renderWeedEffects(ctx, camera)` renders weed leaf particles (physics-based, separate from icon effects); `harvestEffects` (icon type) are batched by Game.render() via `renderEffectsMulti()`.
...
- **TileOverlayManager.js** - Manages sprite overlays on tiles (holes from digging, path edge sprites). `unmarkPathTile(tileX, tileY)` — removes a path tile from the overlay set and recalculates edge overlays for all 4 neighbors (called when player pickaxes a path tile).
...
- **BuildingRegistry.js** - Static configuration for all player-buildable structures and villager milestones. `BUILDING_DEFS` maps string keys to building definitions with fields: `id`, `name`, `category` (`'house'|'special'`), `tilemapPrefix` (CSV path prefix), `footprint` (`{width,height}`), `layers` (array of `{csvSuffix, renderPass}`; renderPass ∈ `'ground'|'upper'|'roof'`), `cost` (`{wood,stone,gold,ore_iron,...}`), `constructionCycles`, `doorOffset` (`{x,y}` local tile), `unlockedBy` (villager id or null), `unique`, `hasTilemap`, `debugOnly`. Current buildings: `small_house` (5×5, hasTilemap:true, tilemapPrefix:'Tileset/house1'), `debug_home` (5×7, debugOnly), plus 14 special buildings (pub, workshop, apothecary, cafe, shrine_temple, forge, trading_post, bakery, dock, goblin_den, theater, laboratory, stable, jewelry_shop, town_hall — all hasTilemap:true, tilemapPrefix:'Tileset/special-test', footprint 5×7, doorOffset {x:3,y:5}). Special buildings layers: Decor (ground), Buildings(Base) (ground), Buildings(Detail) (upper), Buildings(Upper) (upper) — **no Ground layer**. `VILLAGER_MILESTONES` array — each milestone: `{id, name, trigger(milestones)→bool, combo:[{id,count}]}`. 15 milestones from innkeeper to mayor, each tied to a BUILDING_DEFS `unlockedBy`. `buildingCostToRefundItems(cost)` converts cost object to `[{resource, amount}]` refund items for JobManager cancel-refund.
- **BuildingManager.js** - Manages all player-placed buildings. `placedBuildings` array; `_layerCache` Map caches CSV layer data per defId. `loadDefinitionLayers(defId)` fetches CSV files async (safe no-op for hasTilemap:false). `placeBuilding(defId, tileX, tileY, state='under_construction')` returns building object with `chimneyTileId` (347 or 349 for small_house, null otherwise). Building states: `under_construction` (passable, rendered at 0.3 alpha) → `inactive` (path-connected required) → `active_empty` → `active_occupied`. `completeBuildingById(id)` → sets state to `'inactive'`, fires `onBuildingCompleted(building)` callback. `deconstructBuilding(id)` removes and returns building. `getBuildingAt(tileX, tileY)` checks footprint. `getFootprintTiles(building)` returns all `{x,y}` tiles in footprint. `isObstacle(tileX, tileY)` — under_construction buildings are passable; completed ones block. `isPlayerInsideBuilding(playerTileX, playerTileY)` — returns matching building or null (used for roof-hiding). `render(ctx, camera, pass, menuBuildingId=null)` renders all buildings for a pass; when `pass==='roof'` and `menuBuildingId` matches a building id, that building's roof is hidden. **Color recoloring**: blue tileset range (rows 9–15, cols 15–34; tileIds 591–994) is recolored per occupant — `_getColorOffset(building)` returns +0 (blue=unoccupied), +512 (green=goblin_elder), +2048 (purple=unique-unlock villager), +1536 (red=regular villager). `renderGhost(ctx, camera, defId, tileX, tileY, valid)` shows 50% alpha placement preview with green/red tint. `renderDebugOverlay(ctx, camera)` draws colored state overlays. `setTileset(image)` sets the tileset image. **Chimney smoke**: `_chimneySmokers` Map (`buildingId → {animator, pauseTimer}`); `onBuildingOccupied(building)` starts chimney smoke for small_houses (loads `Elements/VFX/Chimney Smoke/chimneysmoke_03_strip30.png`); `onBuildingVacated(building)` stops it; `updateChimneys(deltaTime)` and `renderChimneys(ctx, camera)` called each frame.
- **PathConnectivity.js** - BFS utility to determine if a tile is connected to the great path strip (y = mainPathY … mainPathY+mainPathGap-1). `playerPlacedPaths` Set (`"tileX,tileY"`) tracks player-placed path tiles for pickaxe-removability (not used for BFS — BFS detects path tiles via tilemap tileId). `isPathTile(x, y)` — true for great path rows and tilemap tiles matching CONFIG.tiles.path IDs. `isConnectedToGreatPath(x, y)` — cached BFS result (returns false if start is not a path tile). `invalidate()` clears cache — call after any path tile change. BFS explores 4-directional neighbors; terminates true when any visited tile is on the great path strip. `getWaypointsToGreatPath(startTileX, startTileY, tileSize)` — BFS returning world-pixel waypoints ordered great-path → startTile (walk-in direction for arriving villagers); returns null if startTile is not a path tile or no connection.
- **VillagerManager.js** - Manages recruited villagers and displacement. `villagers` array `[{id, type, houseId}]`. `displacedQueue` FIFO of villager type strings awaiting new houses. `pendingDebugVillagers` counter for "Add Villager (Housed)" debug requests. `getEligibleMilestoneIds()` — checks `game.milestones` against each VILLAGER_MILESTONE trigger, excludes already-recruited types. `onHouseReady(building)` — if displaced villager waiting, assigns immediately; if debug villager pending, calls `travelerManager.spawnDebugVillager()`; else pushes to `_readyHouses` and notifies `game.travelerManager.onEmptyHouseAvailable(building)`. `onVillagerRecruited(villagerType, building, isReturn=false)` — adds villager, sets building to `active_occupied`; if `!isReturn` calls `game.incrementMilestone('totalVillagersRecruited')`; calls `toolbar.refreshBuildSubmenu()` and `game.onBuildingOccupied(building)`. `_detachVillagerFromBuilding(building)` — shared helper: pushes occupant to displaced queue, removes from villagers, calls `game.onBuildingVacated(building)`. `onHouseDeconstructed(building)` and `onHouseDisconnected(building)` both delegate to `_detachVillagerFromBuilding()`. `getVillagerCount()`, `hasVillagerType(type)`. `getUnlockedSpecialBuildings()` — returns defIds of hasTilemap:true special buildings whose `unlockedBy` matches a recruited villager.
...
- **TravelerManager.js (updated)** - Now supports milestone travelers and debug villagers. `villagerManager` reference (set via `setVillagerManager(vm)`). `regularTravelersSinceMilestone` counter. `_pendingEmptyHouse` set by `onEmptyHouseAvailable(building)`. On spawn tick: if pendingEmptyHouse + eligible milestones + counter ≥ `CONFIG.villagers.maxRegularTravelersBeforeMilestone` → spawns milestone traveler (`_spawnMilestoneTraveler(villagerMilestoneId, house)`). Milestone traveler: `isMilestone:true`, `villagerType`, `villagerName`, `targetHouse`, `comboItems:[{id,count}]` — combo items are required at the stand; visiting triggers recruitment on completion. `_createBaseTraveler(forceDirection?)` — shared helper used by `_spawnTraveler()`, `_spawnMilestoneTraveler()`, and `spawnDebugVillager()` to compute leftPx/rightPx, margin, direction, spawnX, despawnX, and hairStyle without duplication. `spawnDebugVillager(building, pathConnectivity)` — spawns a `isDebugVillager:true` traveler on the great path who walks to the target house following player-placed path tiles (using `pathConnectivity.getWaypointsToGreatPath()`); on arrival claims the house via `villagerManager.onVillagerRecruited('debug_villager', house)`.
...
- **Chimney smoke**: farm house chimney at world tile (18, 38); additional chimneys per occupied `small_house` building managed by `BuildingManager._chimneySmokers`
...
- **Well**: tileX=23, tileY=39 (2-wide × 3-tall; obstacles at y=40–41; directly east of house path column x=22)
- **Shed** (shed1.tmx, 5×4): tileX=25–29, tileY=38–41. Bottom row y=41 directly above E-W path. Loaded in `TilemapRenderer.generateChunkMap()`. Ground+Wall layers render below characters; Roof+Roof Detail always render above characters (never hidden). `openStorage` interactable registered — clicking shed opens Storage UI. Wall collision via `collisionRects` (perimeter tiles, `> 0` check); pathfinding blocked via `isBoundary()`. Flower spawning excluded via `isCustomTilemapTile()`.
...
- **House front E-W**: y=42, x=16–28 (extends past well column x=23–24 to shed front at x=25–29)
...
// Phase 2 additions:
well: Well                // Well instance (tileX=23, tileY=39)
...
// Phase 4b additions:
townChunksUnlocked: 0               // number of town plot chunks claimed so far (thresholds: 1,3,6,9,12…)
goblinHouseBuilding: null           // small_house building claimed by goblin when hired
_goblinNeedsHouse: false            // true when goblin hired but no active_empty house available yet
_goblinIdleTimer: 0                 // ms since goblin last had a job (triggers home-return walk)
// _ejectWorkerFromBuilding(workerId, building) — BFS outward from footprint to find walkable tile; teleports worker there
// _deconstructBuilding(building) — displaces villager, ejects workers, removes building, refunds items
// Building click → uiManager.openBuildingMenu(building, def, onDeconstruct)
// buildingManager.updateChimneys(dt) / renderChimneys(ctx, camera) called each frame for occupied small_houses
```
...
10. `replenishZoneManager.render()` — zone perimeter borders (green/grey); drawn before depth-sorted entities
11. Depth-sorted entities (crops, flowers/weeds, trees, ore veins, characters, enemies, effects) — crop ground tiles (`renderCropGroundTile`) rendered inline per crop before `renderCrop` (no separate pre-pass)
12. Forest foregrounds (tree crowns; render OVER great path)
13. `chunkManager.renderPurchaseSigns()` — purchase "?" signs ABOVE trees (north chunks show NO signs)
14. `tilemap.renderUpperLayers()` — store/home upper building layers
15. `tilemap.renderRoofLayers()` — new house roof (hidden when player inside)
16. Chimney smoke (when player outside)
17. `renderEffectsMulti()` — all floating icon effects batched in one save/restore pass (crop, ore, tree, forest chop, flower harvest, seed drop effects)
18. `flowerManager.renderWeedEffects()` — weed leaf particles (physics-based, separate pass)
19. UI
...
- **Tileset/special-test.tmx** - Special building tilemap (5×7). Layers: Decor, Buildings (Base/Detail/Upper). No Ground layer. Used by all 14 special buildings + debug_home in BUILDING_DEFS. Door at local (3,5); path connects to (3,6).
- **Tileset/shed1.tmx** - Farm storage shed (5×4). Layers: Ground, Wall (ground pass), Roof, Roof Detail (roof pass). `openStorage` interactable. Placed statically at x=25–29, y=38–41 in farm chunk. Loaded by TilemapRenderer.generateChunkMap().
- **Elements/chimney/** - `chimneysmoke_02_strip30.png` (300×30, 30 frames) — used by farm house
- **Elements/VFX/Chimney Smoke/** - `chimneysmoke_03_strip30.png` (30 frames) — used by BuildingManager for occupied small_house buildings
...
**CONFIG.villagers** - `maxRegularTravelersBeforeMilestone: 5` — regular travelers spawned before a milestone traveler can spawn. `townExpansionBase: 1` — housed villagers required for 1st town chunk claim. `townExpansionIncrement: 3` — additional housed villagers per subsequent claim (sequence: 1, 3, 6, 9, 12…)
...
**Acceptable tiles**: Defined in TileSelector.js ACCEPTABLE_TILES object per tool. Also checks for resource occupancy (trees, ore, enemies). Hoe extra exclusions: ore vein tiles always block hoe; farm base area (x=15–29, y=34–42) always blocks hoe — player can only hoe at y≥43 on the initial farm chunk.
...
- **North forest flanking chunks** (rows 0–1, flanking cols at col=0,2): start LOCKED. Row 1 flanking chunks become PURCHASABLE as town plots (adjacent to TOWN chunk) once villager thresholds are met. Row 0 flanking chunks also become town plots if the row 1 flanking chunk is TOWN. No purchase signs use gold for town plots — they show villager threshold. Farm forest expansion (`_updatePurchasableChunks()`) only promotes chunks where `row >= farmRow` (2).
